...
void saveProject(char *filename,int n, int m, pin pcb[][40]){
    printf("\n Layout disimpan dalam berkas %s_layout.csv file",filename);
    FILE *fp;
    char *nam;
    nam = filename;
    filename = strcat(filename,"_layout.csv");
    fp = fopen(filename,"w+");
    int i,j;
    for (i=0 ; i<n; i ++){
    	for (j=0; j<m; j++){
	...
	printf("\n Routing disimpan dalam berkas %s_routing.csv file",nam);
    FILE *fp1;
    char nma;
    filename = strcat(nam,"_routing.csv");
    fp1 = fopen(filename,"w+");
    
    for (i=0 ; i<n; i ++){
    	for (j=0; j<m; j++){
		 
    	fprintf(fp1,"%c;", pcb[i][j].simbol );
		}
    	fprintf(fp1,"\n");
	}
    fclose(fp1);
