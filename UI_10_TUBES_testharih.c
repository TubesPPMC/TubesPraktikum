...
//PROGRAM MEMASUKAN SERTA MEMPROSES SEBUAH PCB DOT MATRIKS VIRTUAL
...
//STRUCT ELEMEN DALAM 1 PIN PCB DOTMATRIKS
typedef struct{
        char nama[3];
        int simbol;
        int netlist;
...
//STRUCT UNTUK MENYIMPAN NETLIST DARI FILE DRC
typedef struct{
        char nama[3];
        char netlist[3][1];
} nl;

//PCB DOT MATRIKS VIRTUAL
pin pcb[40][40];
...
//NILAI NETLIST TERBESAR
int netlist_terakhir;

//VARIABLE MEMPROSES NETLIST
int pilihan_netlist;
int temp_int;
char simbol_terpakai[];

//FUNGSI MENGECEK KARAKTER ROUTING SUDAH DIPAKAI ATAU BLUM
...
//FUNGSI ROUTING MANUAL
//pada saat melakukan routing, user measukan routing sesuai denga netlist
//1 netlist 1 simbol
//1 netlist bisa disaambung
...
//mengecek pin tersebut sudah terisi nama part atau belum
...
//FUNGSI MENGECEK APAKAH PADA PIN TERSEBUT SUDAH TERISI SIMBOL ROUTING ATAU BLUM
...
//FUNGSI MENGECEK KEBENARAN INPUT KOMPONEN PENGGUNA
...
//FUNGSI UNTUK MEYIMPAN DATA PADA FILE
...
void saveProject_Net(char *fil,int n, int m, pin pcb[][40]){
	//inisiasi file
	printf("\n Netlist disimpan dalam berkas %s fie",fil);
    FILE *fp;
    char *net;
    net = strcat(fil,"_net.csv");
    fp = fopen(net,"w+");
    //memasukan nilai netlist pada file
    for (i=0 ; i<40; i ++){
    	for (j=0; j<40; j++){
...
void loadProject_layout(pin pcb[][40], char *st){
	...
	if (fp == NULL)
    {
   	...
   	char c;
   	//csv tidak memiliki eof, karena ada kemungkinan bagian tengah file tidak mengandung isi, 
	//maka dari itu, kami melakukan save dan load pada kapasitas maksimum, namun menampilkannya 
	//sesuai dengan input user 
   	//tentukan batas dari csv
    int bound = 1640;
   	//counter char yang diload
	int w;
	i= 0;
	k=0;
	j=0;
	w=0;
	//temp untuk menyimpan string
	char temp[5];
	//selama jumlah char yang di load lebi kecil dari batas
	while( w < bound ){
		c=fgetc(fp);
		w++;
		switch (c) {
	...
	//jika j masih belum maksimum, tambah batas
		if (j<38 ){ 
			if ( w> bound-1){
				bound = bound +1 ;
			//	printf("berhasil tambah boundaries\n");
				} 
			}
		}
...
void loadProject_rout(pin pcb[][40] ,char *st ){
	...
	k=0;
	j=0;
	//temp untuk menyimpan char hasil load
	char temp;
	
	//selama counter lebih kecil dari batas
	while( (w<bound)){
		c=fgetc(fp);
		w++;
		switch (c) {
...
void loadProject_net(pin pcb[][40], char *st){
    ...
    //tentukan batas dari csv
    int bound = 1600;
    //inisiasi counter char
   	int w, net;
	//inisasi hasil untuk netlist terakhir
	int hasil=0;
	i= 0;
	k=0;
	j=0;
	w=0;
	//inisiasi temp untuk integer
	int temp;
	//selama counter kurang dari batas
	while( w < bound ){
		c= fgetc(fp);
		net = c - '0';
		//printf("data ke %d = %d\n",w,net);
...
void drc(){
            ...
            if((net[i]==net[j])&&(i!=j)){
                for (k1=0;k1<n;k1++){
                    for ( k2=0;k2<m;k2++){
                        for (k3=0;k3<n;k3++){
                            for ( k4=0;k4<m;k4++){
                                if((k2!=k4)&&(k1!=k3)&&pcb[k1][k2].simbol==pcb[k3][k4].simbol){
                                    cek++;
                                    angka=cek;
                                }
                            }
    ...
    printf("Jika ada error, hasil derc akan berupa float\n");
    printf("%d Rule is Checked\n",hero);
}
//SECTOR UI
//PROGRAM MENU AWAL
void menuawal(){
...
int menuMode (){
	...
	printf("\t8. Tampilkan Pesebaran Netlist Rangkaian\n");
...
//			printf("manualRout(...);\n");
		...
		case 5: {
			...
			int pilih;
//			printf("autolayout(...);\n");
			printf("[Tipe-Tipe Rangkaian yang tersedia]\n");
			printf("1 Rangkaian Common Emmitor dengan Resistor\n");
			printf("2 Rangkaian Common Emmitor dengan Sumber Arus\n");
			printf("3 Rangkaian Common Emmitor dengan Sumber Arus_dan_Kapasitor_Bypass\n");
			printf("Masukan Tipe yang diinginkan : ");
			scanf("%d",&pilih);
			while (!((pilih ==1 )||(pilih == 2)||(pilih == 3)))
			{
				printf("Masukan tidak ada di pilihan\n");
				printf("1 Rangkaian Common Emmitor dengan Resistor\n");
				printf("2 Rangkaian Common Emmitor dengan Sumber Arus\n");
				printf("3 Rangkaian_Common_Emmitor_dengan_Sumber_Arus_dan_Kapasitor_Bypass\n");
				printf("Masukan Tipe yang diinginkan : ");
				scanf("%d",&pilih);
			}
			if (pilih == 1)
			{
				strcpy (ctr,"Rangkaian_Common_Emmitor_dengan_Resistor");
			}else if (pilih == 2)
			{
				strcpy(ctr, "Rangkaian_Common_Emmitor_dengan_Sumber_Arus");
			}else if (pilih == 3)
			{
				strcpy(ctr, "Rangkaian_Common_Emmitor_dengan_Sumber_Arus_dan_Kapasitor_Bypass");
			}
			loadProject_layout(&pcb,ctr);
	...
	//		printf("autorout(...);\n");
			...
			char *ctr1;
			strcpy(ctr1,ctr);
			loadProject_rout(&pcb,ctr);
			loadProject_net(&pcb,ctr1);
			return 1;
			break;
		}
		case 7: {
